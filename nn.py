...
import numpy as np
import pandas as pd
from utils import hidden_layer
...
#W = tf.get_variable(name='W', shape=[7,7], initializer=tf.contrib.layers.xavier_initializer())
W = tf.Variable(tf.truncated_normal([7, 7], stddev=0.1))
b = tf.Variable(tf.constant(0.1, shape=[1, 7]))


#w1 = tf.get_variable(name='W1', shape=[7,2], initializer=tf.contrib.layers.xavier_initializer())
w1 = tf.Variable(tf.truncated_normal([7, 2], stddev=0.1))
...
cost = tf.reduce_mean(diff) + (tf.nn.l2_loss(W) +tf.nn.l2_loss(w1))*0.0099 + 1.0
...
with tf.Session() as sess:
    ...
    for epoch_i in range(num_of_epochs):
        ...
        with open("Output3.txt", "a") as text_file:
            text_file.write(
                 "epoch: {}, roc_train: {}, roc_test: {}, acc_train: {}, acc_test: {}\n".format( str(epoch_i + 1),
                                                                                                    str(auc_train),
                                                                                                    str(auc_test),
                                                                                                    str(acc_train),
                                                                                                    str(acc_test)
